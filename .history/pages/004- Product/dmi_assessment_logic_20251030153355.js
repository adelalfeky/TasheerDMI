...
function finalizeAssessment() {
    ...
    // ... (existing code for building detailedScoreBody remains the same)

    // Calculate Final Maturity Percentage (This calculation is correct)
    const maturityPercentage = (totalScore / maxScore) * 100; 
    const percentageText = maturityPercentage.toFixed(2) + "%";
    ...
    // Display results
    document.getElementById('maturityScoreDisplay').textContent = percentageText; // Should now show the percentage correctly
    document.getElementById('totalScoreText').textContent = totalScore;
    document.getElementById('maxScoreText').textContent = maxScore;
