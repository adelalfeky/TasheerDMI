...
let liveChartInstance = null;
let finalChartInstance = null;
let finalDoughnutInstance = null; // New instance for the report doughnut chart
let adminChartInstance = null;
...
function updateLiveScore(user) {
    ...
    // Crucial: Use the logged-in user's department to access *their* local storage key
    const storageKey = STORAGE_KEY_PREFIX + user.department.replace(/\s/g, '_'); 
    const selections = JSON.parse(localStorage.getItem(storageKey)) || {};
    ...
    return { totalScore: currentTotalScore, answeredQuestions: currentAnsweredQuestions, maturityPercentage: livePercentage };
}
...
function saveSelection(selectElement) {
    ...
    // Use the logged-in user's department for the storage key
    const storageKey = STORAGE_KEY_PREFIX + user.department.replace(/\s/g, '_'); 
    let selections = JSON.parse(localStorage.getItem(storageKey)) || {};
    const questionId = selectElement.name;
    const selectedValue = selectElement.value;
...
function loadAssessment() {
    ...
    // Display the logged-in user's department name
    document.getElementById('welcomeMessage').textContent = `Welcome, ${user.department}!`;

    buildForm();
    
    // Load data based on the logged-in user's department
    const storageKey = STORAGE_KEY_PREFIX + user.department.replace(/\s/g, '_');
    const savedData = localStorage.getItem(storageKey);
    ...
    // Update live score based on loaded data
    updateLiveScore(user);
}
...
function finalizeAssessment() {
    const user = checkLoginGuard();
    const { totalScore, answeredQuestions, maturityPercentage } = updateLiveScore(user);
    ...
    // Generate both charts for the report
    generateFinalBarChart(maturityPercentage.toFixed(2));
    generateFinalDoughnutChart(maturityPercentage); // New call
}
...
/**
 * NEW FUNCTION: Generates a Doughnut Chart for the final report.
 */
function generateFinalDoughnutChart(maturityPercentage) {
    if (finalDoughnutInstance) {
        finalDoughnutInstance.destroy();
    }
    ...
    const ctx = document.getElementById('finalDoughnutChart').getContext('2d');
    finalDoughnutInstance = new Chart(ctx, config);
}
